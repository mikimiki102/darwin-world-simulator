...
public class PlantGenerator {
    ...
    private void freeAll() {
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < jungleMinY; y++) {
                freeStepFields.add(new Vector2d(x, y));
            }
            for (int y = jungleMinY; y <= jungleMaxY; y++) {
                freeJungleFields.add(new Vector2d(x, y));
            }
    ...
    public Vector2d getPosition() {
        ...
        if (freeJungleFields.isEmpty()) {
            return freeStepFields.removeRandom(random);
        }
        if (freeStepFields.isEmpty()) {
            return freeJungleFields.removeRandom(random);
        }
